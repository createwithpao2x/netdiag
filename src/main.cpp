...
void interfaceBrief(ofstream &outFile) {
    ...
    for (const auto& iface : interfaces) {
        std::cout << "Interface: " << iface.interface_name << std::endl;
        std::cout << "IP Address: " << iface.ip_address << iface.subnet_mask << std::endl;
        std::cout << "Gateway: " << iface.gateway << std::endl;
        std::cout << "DNS Servers: " << iface.dns_servers << std::endl;
        std::cout << "DHCP Enabled: " << (iface.dhcp_enabled ? "Yes" : "No") << std::endl;
        std::cout << std::endl;
        writeToFile(outFile, "Interface: " + iface.interface_name);
        writeToFile(outFile, "IP Address: " + iface.ip_address + " " + iface.subnet_mask);
        writeToFile(outFile, "Gateway: " + iface.gateway);
        writeToFile(outFile, "DNS Servers: " + iface.dns_servers);
        writeToFile(outFile, "DHCP Enabled: " + (iface.dhcp_enabled ? string("Yes") : string("No")));
    }
}
...
void tracerouteTest(const string& target, ofstream &outFile) {
    ...
    cout << "TRACEROUTE TEST RESULTS (" << target << ")\n";
    cout << "Lines with the Most Latency:\n";
    writeToFile(outFile, "\nTRACEROUTE TEST RESULTS (" + target + ")");
    writeToFile(outFile, "Lines with the Most Latency:");
    int count = 0;
    ...
    cout << "Total Hop Count: " << hopLines.size() << "\n" << endl;
    writeToFile(outFile, "Total Hop Count: " + to_string(hopLines.size()));
}

void pingTest(const string& target, ofstream &outFile) {
    int iterations = 30;
    int minPing = INT_MAX, maxPing = 0, avgPing = 0;
    int packetsSent = 0, packetsReceived = 0, packetsLost = 0;
    vector<int> pingTimes;
    for (int i = 0; i < iterations; i++) {
        string cmd = "ping -n 1 " + target + " | findstr /i \"time=\"";
        string result = executeCommand(cmd);
        if (!result.empty()) {
    ...
    cout << "PING TEST RESULTS (" << target << ")\n";
    cout << "Minimum Ping: " << minPing << " ms | Maximum Ping: " << maxPing << " ms | Average Ping: " << avgPing << " ms\n";
    cout << "Packets Sent: " << packetsSent << " | Packets Received: " << packetsReceived << " | Packet Loss: " << packetsLost << " (" << packetLossPercentage << "%)\n\n";
    writeToFile(outFile, "\nPING TEST RESULTS (" + target + ")");
    writeToFile(outFile, "Minimum Ping: " + to_string(minPing) + " ms | Maximum Ping: " + to_string(maxPing) + " ms | Average Ping: " + to_string(avgPing) + " ms");
    writeToFile(outFile, "Packets Sent: " + to_string(packetsSent) + " | Packets Received: " + to_string(packetsReceived) + " | Packet Loss: " + to_string(packetsLost) + " (" + to_string(packetLossPercentage) + "%)");
    if (pingTimes.size() > 1) {
        ...
        cout << "JITTER TEST RESULTS (" << target << ")\n";
        cout << "Jitter: " << jitter << " ms\n" << endl;
        writeToFile(outFile, "\nJITTER TEST RESULTS (" + target + ")");
        writeToFile(outFile, "Jitter: " + to_string(jitter) + " ms");
    } else {
        cout << "Network Unstable.\n" << endl;
        writeToFile(outFile, "Network Unstable.");
    }
}

void processIPFromFile(const string& filename, ofstream &outFile) {
    ifstream file(filename);
    if (!file) {
        cerr << "iplist.txt: File Not Found!" << endl;
        return;
    }
    string targetIP;
    while (getline(file, targetIP)) {
        if (!targetIP.empty()) {
            targetIP.erase(0, targetIP.find_first_not_of(" \t"));
            targetIP.erase(targetIP.find_last_not_of(" \t") + 1);
...
int main() {
    ...
    processIPFromFile(filename, outFile);
    outFile.close();
    return 0;
}